...
fetched between rounds. The block is a heading carrying a status tag, one
sentence of context, an options table, exactly one recommendation, and the
...
A resolution is **appended below the recommendation, never written over it**,
and names the option, who chose it, the date, and the reason where the
recommendation was overridden. A `Q` resolved without the user is marked
`provisional` and re-raised in `## What I need from you`.

Discussion is iterative and the file must show it: append one row to
`## Discussion log` per round. Between rounds, go and get the evidence rather
than arguing from memory -- a round that changes no evidence usually changes no
minds.

The brainstorm reaches `SETTLED` when every `Q` is `RESOLVED`, `DEFERRED`, or
`DROPPED`. Then, and only then, write the plan.

A worked `Q` block, the resolution and discussion-log formats:
[references/question-lifecycle.md](./references/question-lifecycle.md)
sections A-C.
...
**Never write a bare value.** Every contested parameter gets a row carrying the
proposed value, **why this value**, **what changes if it moves**, and a
**source** that is a file path, a tool version, or a citation -- never "standard
practice". A parameter that cannot be settled before seeing data says so and
names the cheap check that will settle it.

**For a tool the project has not used**, establish the ground truth in this
order: what version is installed (from the binary, not the docs); what modes it
has (`--help` on that binary); **what it does not do** -- the highest-value
paragraph in the document; what its input contract is (orientation, coordinate
base, index base, chromosome naming, effect allele -- every one of these can
produce zero results with no error); what it costs to run, measured; and what
would validate it. Where two engines will run, say which is primary, which is
the cross-check, and what a disagreement would mean.

The parameter-row template and the six-step software workup:
[references/question-lifecycle.md](./references/question-lifecycle.md)
sections D-E. Per-analysis question sets -- GWAS, QTL, EWAS, RNA-seq,
single-cell, multi-omics -- and the parameters that always need justification:
