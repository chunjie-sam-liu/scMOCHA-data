...
fn_plot_disease_variant_bar <- function(df_wide, annotation = NULL) {
  ...
  color_disease_other <- c(
    color_disease,
    "Other disease" = color_disease[["Other"]]
  )
  ...
  p <- df_long |>
    ...
    scale_fill_manual(name = "Disease", values = color_disease_other) +
    geom_hline(
...
disease_meta |>
  ...
  ) -> plot_other_disease_chemistry

saveplot(
  filename = outdirnotuse /
    "Other-disease" /
    "Other-disease-DISEASE-CHEMISTRY-DISTRIBUTION.pdf",
  plot = plot_other_disease_chemistry,
  width = 6,
  height = 6,
  create.dir = TRUE
)
...
meta_af |>
  export(outdirnotuse / "Other-disease" / "Other-disease-variant-af.qs")

fn_plot_disease_variant_bar(
  variant_nsamples_wide,
  disease_variant_annotation
...
saveplot(
  ...
  filename = outdirnotuse /
    "Other-disease" /
    "Other-disease-variant-distribution-ranked.pdf",
  width = 17,
  height = 8,
  create.dir = TRUE
)
