...
# @DESCRIPTION: Compare COVID-19 vs Healthy within each celltype using all variant-carrying cells from GSE166992.
# @VERSION: v0.0.1
...
main <- function() {
  ...
  # Try to reuse merged Seurat from 17.07 (GSE166992-specific)
  ...
  # GSE166992 only has COVID-19 and Healthy
  comparison <- list(
    key = "covid",
  ...
  summary_all <- rbindlist(lapply(names(level_jobs), function(level_name) {
  ...
  log_info("GSE166992 COVID-19 vs Healthy comparison outputs saved to {root_outdir}")
