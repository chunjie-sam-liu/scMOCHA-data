...
fn_load_count <- function(thepath, type = c("cluster", "cell")) {
  ...
  tibble::tibble(
    ...
    dplyr::mutate(d = parallel::mclapply(
      path, data.table::fread,
      mc.cores = 4
      )) |>
...
barcode <- data.table::fread("/mnt/isilon/u01_project/large-scale/ting/raw/GSE233304/Running_results/GSM8244396/barcode_cluster.tsv", col.names = c("barcode", "cj", "celltype"))
...
reads_dt |>
  dplyr::filter(
    ...
    (
      ...
      (
        is.na(tag.nM) | as.numeric(tag.nM) <= NMmax | as.numeric(tag.nM) <= NMmax
      )
...
# save image --------------------------------------------------------------
gse_data <- import("/home/liuc9/github/scMOCHA-data/analysis/zzz/clean-data/gse_data.qs")