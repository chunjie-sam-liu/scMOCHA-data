...
def load_mgatk_output(output_dir, mito_length, sample_prefix):
    ...
    base_files = [
        glob.glob(f"{output_dir}/{sample_prefix}.{nt}.txt.gz")[0]
        for nt in "ATCG"
    ]
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = {
            executor.submit(process_base_file, base_file, mito_length): nt
            for base_file, nt in zip(base_files, "ATCG")
        }
...
def gather_possible_variants(base_coverage_dict, reference_file):
    ...
    ref_set = [x.strip().split() for x in open(reference_file, "r")]
    ref_N_positions = [
        int(x[0]) for x in ref_set if x[1].upper() not in letters
    ]
    ref_set = {
...
base_coverage_dict = load_mgatk_output(
    MGATK_OUT_DIR, mito_length, sample_prefix
)
...
variants = gather_possible_variants(
    base_coverage_dict, f"{MGATK_OUT_DIR}/{mito_genome}_refAllele.txt"
)
...
fwd_cell_variant_df = pd.DataFrame(
    np.array(fwd_cell_variant_df).T, index=cell_barcodes, columns=variant_names
)
rev_cell_variant_df = pd.DataFrame(
    np.array(rev_cell_variant_df).T, index=cell_barcodes, columns=variant_names
)
...
mask_idx = (
    fwd_cell_variant_df + rev_cell_variant_df
) == 0  # set 0 on both strands to nan to exclude from correlation calculation
...
variant_nucleotide = [f"{x[1]}>{x[2]}" for x in variants]
...
heteroplasmic_df.to_csv(
    MGATK_OUT_DIR + sample_prefix + ".cell_heteroplasmic_df_raw.tsv.gz",
    sep="\t",
    compression="gzip",
)
...
multi_cell_variants = variant_output[
...
# plt.scatter(variant_output[variant_output["variant"].isin(multi_cell_variants)]["strand_correlation"], np.log10(variant_output[variant_output["variant"].isin(multi_cell_variants)]["vmr"] + 0.1), s=5)
