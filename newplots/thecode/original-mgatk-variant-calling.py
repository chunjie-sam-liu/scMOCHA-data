...
matplotlib.use(
    "Agg"
)  # this runs headless as part of the pipeline; never needs a GUI backend
import numpy as np
import pandas as pd
...
def _load_mgatk_output(output_dir, mito_length):
    ...
    for i, nt in enumerate("ATCG"):
        ...
        missing_pos = [
            x for x in range(1, mito_length + 1) if x not in fwd_base_df.columns
        ]
        ...
        missing_pos = [
            x for x in range(1, mito_length + 1) if x not in rev_base_df.columns
        ]
...
def call_variants(
    ...
    variants = _gather_possible_variants(
        base_coverage_dict,
    ...
    # strand correlation
    mask_idx = (
        (fwd_cell_variant_df + rev_cell_variant_df) == 0
    )  # set 0 on both strands to nan to exclude from correlation calculation
    fwd_cell_variant_df[mask_idx] = np.nan
    rev_cell_variant_df[mask_idx] = np.nan
    variant_strand_corr = fwd_cell_variant_df.corrwith(
        rev_cell_variant_df
    ).round(3)
    ...
    variant_n_cells_conf_detected = (
        (fwd_cell_variant_df >= 2) & (rev_cell_variant_df >= 2)
    ).sum()
    ...
    plt.scatter(
        variant_output[variant_output["variant"].isin(multi_cell_variants)][
            "strand_correlation"
        ],
        np.log10(
            variant_output[variant_output["variant"].isin(multi_cell_variants)][
