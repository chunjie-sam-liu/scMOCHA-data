...
fn_save_somatic_table_bar <- function() {
  source(
    path(Sys.getenv("HIGHRESDIR")) / "00-colors.R"
  )
  SOMATIC_VARIANTS |>
    select(srrid, variant, B:other_T) -> d
  # d |> group_by(variant) |> filter(n() > 1) |> arrange(variant)
  d |>
    pivot_longer(
      cols = -c(srrid, variant),
      names_to = "celltype",
      values_to = "af"
    ) |>
    mutate(
      ismutated = if_else(af >= 0.05, 1, 0)
    ) |>
    group_by(
      celltype
    ) |>
    summarize(
      nmutated = sum(ismutated, na.rm = TRUE),
      total = n(),
      prop = nmutated / total
    ) |>
    mutate(
  ...
  saveplot(
    filename = path(
      ...
      "Somatic-mutation-occurence.pdf",
    plot = p,
    width = 6,
    height = 4
  )
