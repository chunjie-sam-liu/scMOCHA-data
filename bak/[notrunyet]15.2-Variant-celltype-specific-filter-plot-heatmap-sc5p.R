...
celltype_specific_variant <- import("/home/liuc9/github/scMOCHA-data/analysis/zzz/clean-data/celltype_specific_variant.qs") |>
  dplyr::filter(srrid %in% srrids) |>
  dplyr::count(
    variant
  ) |>
...
VARIANTS <- heteroplasmic |>
  dplyr::filter(variant %in% celltype_specific_variant$variant) |>
  dplyr::pull(variant)
length(VARIANTS)
...

{
  pdf(
    file = file.path(
      ...
      "heatmap_cluster_af_SC5P-PE_AD.pdf"
    ),
    width = 20,
    height = 9
  )
