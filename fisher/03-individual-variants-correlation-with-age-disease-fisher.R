...
anno_meta_info_clean <- gse_dataset_metadata_full |>
  dplyr::mutate(
    `# of variants` = n_heteroplasmic + n_homoplasmic,
    `# of somatic variants` = n_heteroplasmic,
    `# of homoplasmic variants` = n_homoplasmic,
  )
    ...
    expr = {
      anno_meta_info_clean |>
        ggpubr::ggscatter(
          ...
          xlab = "",
          ylab = "Number of Heteroplasmic Variants",
          color = "Chemistry",
          add = "reg.line",
      ...
      anno_meta_info_clean |>
        dplyr::mutate(
          disease = dplyr::case_when(
            disease %in%
              c("Alzheimer's Disease", "Healthy", "COVID-19", "Unknown") ~
              disease,
            TRUE ~ "Other"
          )
        ) |>
...
ggsave(
  path = zzz_out,
  filename = "ggpubr_correlation_somatic_variants-fisher.pdf",
...
ggsave(
  path = zzz_out,
  filename = "ggpubr_correlation_somatic_variants_cutoff2-fisher.pdf",
  plot = fn_eda_ggpubr(
...
ggsave(
  path = zzz_out,
  filename = "ggpubr_correlation_somatic_variants_cutoff1-fisher.pdf",
  plot = fn_eda_ggpubr(
...
anno_meta_info_clean |>
  ...
  labs(
    title = "Number of Heteroplasmic Variants correlates with Age",
    x = "Age",
    y = "# of somatic variants"
  ) -> p_age_cor
ggsave(
  path = zzz_out,
  filename = "ggpubr_correlation_age_somatic_variants-fisher.pdf",
  plot = p_age_cor,
  width = 8,
  height = 6
)
...
ggscatterstats(
  ...
  xlab = "Age",
  ylab = "Number of Heteroplasmic Variants",
  title = "Relationship between Age and Number of Heteroplasmic Variants",
  xfill = "#CC79A7",
  yfill = "#009E73",
  marginal = TRUE,
...
ggsave(
  path = zzz_out,
  filename = "ggpubr_ggstatsplot_correlation_age_somatic_variants-fisher.pdf",
...
ggsave(
  path = zzz_out,
  filename = "ggpubr_ggstatsplot_correlation_sex_somatic_variants-fisher.pdf",
  plot = p_collect,
  width = 8,
  height = 5
)
